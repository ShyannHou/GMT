...
for run in range(10):
    ...
    for time_slot in range(num_tasks):
        with open(datapath+f'sub_graph_{time_slot}_by_edges', 'rb') as file:
            g = pickle.load(file)
        n_nodes = g.num_nodes()
        n_new_nodes = sum(g.ndata['new_nodes_mask'])
        new_node_idxs = g.ndata['new_nodes_mask'].nonzero()
        ...
        ind_train, ind_valid, ind_test =        shuffled_ind[:n_train], shuffled_ind[n_train:n_train+n_valid], shuffled_ind[n_train+n_valid:n_new_nodes]
