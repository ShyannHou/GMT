...
def csr2tensor(Acsr, device=None):
    Acoo = Acsr.tocoo()
    if device is None:
        device = torch.device("cpu")
    return torch.sparse.FloatTensor(
        torch.LongTensor([Acoo.row.tolist(), Acoo.col.tolist()]),
        torch.FloatTensor(Acoo.data.astype(np.int32)),
    ).to(device)
...
def combine_graph(g, Gc=None, C=None, c2n=None, n2c=None, device=None):
    ...
    if Gc == None:
        ...
        node_idxs = g.ndata['node_idxs'].to(device)
        n2c[node_idxs] = torch.tensor([i for i in range(n_nodes)], device=device)
        return g, c2n, n2c
    ...
    if 'topo_feat' in g.ndata:
        node_data['topo_feat'] = g.ndata['topo_feat'][new_node_idxs].float()

    Gc.add_nodes(num_new_nodes, node_data)

    max_new_node_idx = torch.max(g.ndata['node_idxs'][new_node_idxs]).item()

    if n2c.size()[0] < max_new_node_idx+1:
        n2c = torch.cat(
            [n2c, torch.tensor([-1 for i in range(max_new_node_idx+1-n2c.size()[0])], device=device)],
            0,
        )
    n2c[g.ndata['node_idxs'][new_node_idxs].to(device)] = torch.tensor(
        [i+len(c2n) for i in range(new_node_idxs.shape[0])], device=device
    )
    ...
    valid_edge_idx = torch.tensor(list(valid_col_idx.intersection(valid_row_idx)), device=device)
...
def graph_coarsening(g, node_hidden_features, c2n, n2c, train_ratio, reduction, replay_nodes, device=None, target_nodes=None):
    ...
    if target_nodes is not None:
        if target_nodes >= N:
            ...
            print(f"[Coarsening] target_nodes ({target_nodes}) >= current nodes ({N}), skipping coarsening")
            reduction = 0.0
        else:

            reduction = max(0.0, min(0.999, 1.0 - target_nodes / N))
            print(f"[Coarsening] target_nodes={target_nodes}, N={N}, computed reduction={reduction:.4f}")

    start_time = time.time()
    C, Gc, Call, Gall = coarsen(G_orig=g_pyg, G=g_pyg, r=reduction, replay_nodes=replay_nodes, feature=node_hidden_features)
    end_time = time.time()
    C_tensor = csr2tensor(C, device=device)
    ...
    if 'topo_feat' in g.ndata:
        ...
        coarsened_topo = C_tensor @ topo_feat
        g_dgl.ndata['topo_feat'] = coarsened_topo
    n2c_new = torch.tensor([-1 for el in range(n2c.size()[0])], device=device)
    ...
    for i in range(C.shape[0]):
        ...
        for super_node in cluster_i:
            ...
            n2c_new[torch.tensor(list(nodes), device=device)] = torch.tensor(
                [i for _ in range(len(nodes))], device=device
            )
    ...
    coarsened_train_mask = torch.zeros(len(c2n_new), device=device).bool()
    coarsened_valid_mask = torch.zeros(len(c2n_new), device=device).bool()
    coarsened_train_mask[torch.tensor(mask_idxs[:num_training_sample], device=device).long()] = True
    coarsened_valid_mask[torch.tensor(mask_idxs[num_training_sample:], device=device).long()] = True
